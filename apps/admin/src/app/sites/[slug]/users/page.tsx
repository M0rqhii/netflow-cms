        ...
        <div className="mb-2 sm:mb-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-1.5">
            ...
            <Link href={`/sites/${encodeURIComponent(slug)}`}>
              <Button variant="outline" className="w-full sm:w-auto text-xs sm:text-sm h-7 sm:h-8 px-2 sm:px-3">{t('common.back')}</Button>
            </Link>
          </div>
        </div>
              ...
              <>
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5 mb-2">
                  <Input
                    ...
                    className="w-full sm:max-w-xs text-xs sm:text-sm h-8 sm:h-9"
                  />
                  <div>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <thead>
                          <tr className="text-left text-muted border-b border-border">
                            <th scope="col" className="py-2 px-3 font-semibold text-xs sm:text-sm">{t('users.email')}</th>
                            <th scope="col" className="py-2 px-3 font-semibold text-xs sm:text-sm">{t('users.role')}</th>
                            <th scope="col" className="py-2 px-3 font-semibold text-xs sm:text-sm">{t('users.joined')}</th>
                          </tr>
                        </thead>
                        <tbody>
                          {filteredUsers.map((u) => (
                              ...
                              <td className="py-2 px-3">
                                {editingUserId === u.id ? (
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    ...
                                    <select
                                      id={`role-edit-${u.id}`}
                                      className="border rounded-md px-1.5 sm:px-2 py-1 text-xs sm:text-sm h-7 sm:h-8 bg-card text-foreground"
                                      value={editingRole}
                                  ...
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <Badge className="text-xs sm:text-xs">{roleLabel(u.role)}</Badge>
                                    <Button
                                      variant="outline"
                                      size="sm"
                    ...
                    <div className="md:hidden space-y-2">
                      {filteredUsers.map((u) => (
                        <div key={u.id} className="border border-border rounded-lg p-2 sm:p-3 space-y-1.5 sm:space-y-2">
                          <div>
                            <div className="text-xs text-muted mb-0.5">{t('users.email')}</div>
                            <div className="font-medium text-xs sm:text-sm">{u.email}</div>
                          </div>
                          <div>
                            <div className="text-xs text-muted mb-0.5">{t('users.role')}</div>
                            {editingUserId === u.id ? (
                              <div className="space-y-1.5">
                                <select
                                  className="border rounded-md px-2 py-1 text-xs sm:text-sm h-7 sm:h-8 w-full bg-card text-foreground"
                                  value={editingRole}
                                  onChange={(e) => setEditingRole(e.target.value)}
                                >
                                  <option value="super_admin">{t('users.superAdmin')}</option>
                                ...
                                <div className="flex gap-1.5">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() => onUpdateRole(u.id, editingRole)}
                                    className="flex-1 text-xs sm:text-sm h-7 sm:h-8 px-2"
                                  >
                                    {t('common.save')}
                                  </Button>
                                  <Button
                          ...
                          <div>
                            <div className="text-xs text-muted mb-0.5">{t('users.joined')}</div>
              ...
              <form onSubmit={onCreateUser} className="space-y-2 w-full max-w-lg">
                ...
                <div className="flex flex-col sm:flex-row justify-end gap-1.5">
                  <Button
                    ...
                    className="w-full sm:w-auto text-xs sm:text-sm h-7 sm:h-8 px-2 sm:px-3"
                  >
                    {t('common.cancel')}
                  </Button>
                  <Button type="submit" variant="primary" disabled={creating} className="w-full sm:w-auto text-xs sm:text-sm h-7 sm:h-8 px-2 sm:px-3">
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="px-3 sm:px-4 pb-2 sm:pb-3">
              <form onSubmit={onInvite} className="space-y-2 w-full max-w-lg">
                ...
                <div className="flex flex-col sm:flex-row justify-end gap-1.5">
                  <Button type="submit" variant="primary" disabled={sending} className="w-full sm:w-auto text-xs sm:text-sm h-7 sm:h-8 px-2 sm:px-3">
                    ...
                    <table className="w-full text-sm">
                      ...
                      <thead>
                        <tr className="text-left text-muted border-b border-border">
                          <th scope="col" className="py-2 px-3 font-semibold text-xs sm:text-sm">{t('users.email')}</th>
                          <th scope="col" className="py-2 px-3 font-semibold text-xs sm:text-sm">{t('users.role')}</th>
                          <th scope="col" className="py-2 px-3 font-semibold text-xs sm:text-sm">{t('users.sent')}</th>
                          <th scope="col" className="py-2 px-3 font-semibold text-xs sm:text-sm">{t('common.actions')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {filteredInvites.map((invite) => (
                          <tr key={invite.id} className="border-b border-border hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                            ...
                            <td className="py-2 px-3">
                              <Badge className="text-xs sm:text-xs">{roleLabel(invite.role)}</Badge>
                            </td>
                  ...
                  <div className="md:hidden space-y-2">
                    {filteredInvites.map((invite) => (
                      <div key={invite.id} className="border border-border rounded-lg p-2 sm:p-3 space-y-1.5 sm:space-y-2">
                        ...
                        <div>
                          <div className="text-xs text-muted mb-0.5">{t('users.sent')}</div>
