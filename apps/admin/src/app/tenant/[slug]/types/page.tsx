...
import { fetchTenantTypes, createType, deleteType, updateType, type TypeSummary } from '@/lib/api';
...
export default function TenantTypesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setTenant(t);
        const rows = await fetchTenantTypes(t.tenantId).catch(() => []);
        setItems(rows);
      } catch (e) {
  ...
  const onCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const created = await createType(tenant.tenantId, { name, slug: tslug });
  ...
  return (
    <div className="container py-8">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold">Content Types · {slug}</h1>
        <div className="flex items-center gap-2">
          {tenant && canEditContent(tenant.role) && (
            <>
              <button className="btn btn-primary" onClick={() => setOpen(true)}>New (modal)</button>
              <button className="btn btn-outline" onClick={() => { setQuickAdd((v)=>!v); setName(''); setTslug(''); setSlugTouched(false); }}>Quick Add</button>
            </>
          )}
      ...
      {quickAdd && tenant && canEditContent(tenant.role) && (
        <div className="card mb-4"><div className="card-body">
          <form
            onSubmit={async (e) => {
              ...
              try {
                const created = await createType(tenant.tenantId, { name, slug: tslug });
                setItems((prev) => [created, ...prev]);
                setName(''); setTslug(''); setSlugTouched(false); setQuickAdd(false);
                push({ tone: 'success', message: 'Type created' });
              } catch (err) {
                push({ tone: 'error', message: err instanceof Error ? err.message : 'Failed to create' });
          ...
          >
            <TextInput autoFocus label="Name" value={name} onChange={(e) => { setName(e.target.value); if (!slugTouched) setTslug(slugify(e.target.value)); }} required />
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" className="btn btn-outline" onClick={() => setTslug(slugify(name))}>Generate</button>
              </div>
            ...
            <div className="md:col-span-2 flex items-center justify-end gap-2">
              ...
              <button type="submit" className="btn btn-primary" disabled={creating || !isValidSlug(tslug)}>{creating ? 'Creating...' : 'Create'}</button>
            </div>
          </form>
        </div></div>
      )}
      ...
      {loading ? (
        ...
        <div className="card"><div className="card-body"><p className="text-muted">No types yet.</p></div></div>
      ) : (
        <div className="space-y-3">
          {items.map((it) => (
              ...
              {tenant && canEditContent(tenant.role) && editItem?.id === it.id ? (
                <form
                  onSubmit={async (e) => {
                    ...
                    try {
                      const updated = await updateType(tenant.tenantId, editItem.id, { name, slug: tslug });
                      setItems((prev) => prev.map((x) => (x.id === editItem.id ? { ...x, ...updated } : x)));
                      setEditItem(null); setName(''); setTslug('');
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <TextInput label="Name" value={name} onChange={(e) => { setName(e.target.value); if (!slugTouched) setTslug(slugify(e.target.value)); }} required />
                    <div>
                      <label className="block text-sm font-medium mb-1">Slug</label>
                      <div className="flex items-center gap-2">
                        <input className={"border rounded w-full p-2 " + (tslug && !isValidSlug(tslug) ? 'border-red-500' : '')} value={tslug} onChange={(e) => { setTslug(e.target.value); setSlugTouched(true); }} required />
                  ...
                  <div className="flex items-center gap-2 justify-end">
                    ...
                    <button type="submit" className="btn btn-primary" disabled={saving || !isValidSlug(tslug)}>{saving ? 'Saving...' : 'Save'}</button>
                  </div>
                </form>
              ) : (
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-muted">{it.slug} {it.fieldsCount ? `· ${it.fieldsCount} fields` : ''}</div>
                  </div>
                  {tenant && canEditContent(tenant.role) && (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        onClick={async () => {
                          try {
                            const path = `/api/v1/content-types/${it.id}`;
                            await navigator.clipboard.writeText(path);
                            push({ tone: 'success', message: 'API path copied' });
                          } catch { push({ tone: 'error', message: 'Cannot copy' }); }
                        }}
                      >Copy API</button>
                      <button className="btn btn-outline" onClick={() => { setEditItem(it); setName(it.name); setTslug(it.slug); setSlugTouched(false); }}>Edit</button>
                      <button className="btn btn-outline" onClick={() => onDelete(it.id)}>Delete</button>
                    </div>
                  )}
                </div>
              )}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="New Type">
        <form onSubmit={onCreate} className="space-y-3">
          <TextInput label="Name" value={name} onChange={(e) => { setName(e.target.value); if (!slugTouched) setTslug(slugify(e.target.value)); }} required />
          <TextInput label="Slug" value={tslug} onChange={(e) => { setTslug(e.target.value); setSlugTouched(true); }} error={tslug && !isValidSlug(tslug) ? 'Use lowercase letters, numbers, and hyphens' : undefined} required />
          <div className="flex items-center gap-2 justify-end">
      ...
      {/* Edit modal removed; inline editing enabled */}
    </div>
  );
}
