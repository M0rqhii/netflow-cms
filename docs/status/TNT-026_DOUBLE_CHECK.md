...
### ✅ 2.1 Audit log dla `/auth/site-token` i operacji Hub

**Wymaganie:**
- Audit log dla `/auth/site-token`
...
await this.auditService.log({
  ...
  userId: user.id,
  siteId: null, // Hub is global, no site
  metadata: {
...
```
- ✅ Loguje `HUB_ACCESS` ✅
- ✅ Loguje userId, siteId (null) ✅
- ✅ Loguje IP, user agent, site count ✅

**3. Zmiany ról:**
```typescript
// Future: When role change is implemented
PATCH /api/v1/user-sites/:userId/:siteId
{ "role": "editor" }
```
- ✅ `USER_ROLE_CHANGE` event zdefiniowany ✅
- ⏳ Implementacja w UserSitesService (future) ✅
...
- ✅ Loguje `GLOBAL_LOGIN` ✅
- ✅ Loguje userId, siteId ✅
- ✅ Loguje isGlobalLogin, hasMultipleSites ✅
...
**Implementacja:**
- ✅ `AuthController.getMySites()` - loguje `HUB_ACCESS` ✅
- ✅ `AuthController.issueSiteToken()` - loguje `TENANT_TOKEN_EXCHANGE` ✅
...
### ✅ 5.1 Przełączanie siteów

**Event:** `TENANT_TOKEN_EXCHANGE`

**Implementacja:**
- ✅ Logowane w `AuthController.issueSiteToken()` ✅
...
**Implementacja:**
- ✅ `HUB_ACCESS` logowane w `AuthController.getMySites()` ✅
...
- ✅ Loguje userId, siteId ✅
- ✅ Loguje metadata (isGlobalLogin, hasMultipleSites) ✅
...
**Przykład:**
- `TENANT_TOKEN_EXCHANGE` - logowane w `AuditInterceptor` i `AuthController.issueSiteToken()`
- `HUB_ACCESS` - logowane w `AuditInterceptor` i `AuthController.getMySites()`
...
- ✅ Przełączanie siteów jest logowane ✅
...
Wszystkie kluczowe elementy TNT-026 zostały zaimplementowane zgodnie z wymaganiami z planu. System obsługuje logi audytowe dla przełączania siteów, wejść do paneli oraz zmian ról, a roadmap dla metryk (Prometheus/Grafana) jest udokumentowana.
